...
unsigned frontend::ComputeLinearIndex(Type *Ty, const unsigned *Indices,
  ...
  if (StructType *STy = dyn_cast<StructType>(Ty)) {
    for (auto I : frontend::enumerate(STy->elements())) {
      Type *ET = I.value();
...
/// ComputeValueVTs - Given an FRONTEND IR type, compute a sequence of
...
///
void frontend::ComputeValueVTs(const TargetLowering &TLI, const DataLayout &DL,
  ...
  if (Ty->isVoidTy())
    return;
  // Base case: we can get an EVT for this FRONTEND IR type.
...
void frontend::computeValueLLTs(const DataLayout &DL, Type &Ty,
  ...
  if (Ty.isVoidTy())
    return;
  // Base case: we can get an LLT for this FRONTEND IR type.
...
GlobalValue *frontend::ExtractTypeInfo(Value *V) {
  V = V->stripPointerCasts();
  GlobalValue *GV = dyn_cast<GlobalValue>(V);
  GlobalVariable *Var = dyn_cast<GlobalVariable>(V);

  if (Var && Var->getName() == "frontend.eh.catch.all.value") {
...
/// the given FRONTEND IR floating-point condition code.  This includes
/// consideration of global floating-point math flags.
///
ISD::CondCode frontend::getFCmpCondCode(FCmpInst::Predicate Pred) {
  switch (Pred) {
  ...
  case FCmpInst::FCMP_TRUE:
    return ISD::SETTRUE;
  default:
    frontend_unreachable("Invalid FCmp predicate opcode!");
  }
}

ISD::CondCode frontend::getFCmpCodeWithoutNaN(ISD::CondCode CC) {
  switch (CC) {
  case ISD::SETOEQ:
  case ISD::SETUEQ:
    return ISD::SETEQ;
...
ISD::CondCode frontend::getICmpCondCode(ICmpInst::Predicate Pred) {
  switch (Pred) {
  ...
  case ICmpInst::ICMP_UGT:
    return ISD::SETUGT;
  default:
    frontend_unreachable("Invalid ICmp predicate opcode!");
  }
}

ICmpInst::Predicate frontend::getICmpCondCode(ISD::CondCode Pred) {
  switch (Pred) {
  case ISD::SETEQ:
  ...
  default:
    frontend_unreachable("Invalid ISD integer condition code!");
  }
}
...
bool frontend::isInTailCallPosition(const CallBase &Call,
...
bool frontend::attributesPermitTailCall(const Function *F, const Instruction *I,
...
bool frontend::returnTypeIsEligibleForTailCall(const Function *F,
                                               const Instruction *I,
                                               const ReturnInst *Ret,
                                               const TargetLoweringBase &TLI,
                                               bool ReturnsFirstArg) {
  ...
  do {
    ...
    SmallVector<unsigned, 4> TmpRetPath(frontend::reverse(RetPath));
    SmallVector<unsigned, 4> TmpCallPath(frontend::reverse(CallPath));
...
bool frontend::funcReturnsFirstArgOfCall(const CallInst &CI) {
...
frontend::getEHScopeMembership(const MachineFunction &MF) {
...
void frontend::EmitJSON(const RecordKeeper &RK, raw_ostream &OS) {
...
public:
  ...
  void learning_rate(const double &learning_rate) {
    learning_rate_ = learning_rate;
  }
  ...
  void train(std::vector<TrainingItem> &training_set,
    ...
    if (max_iterations == 0)
      throw std::invalid_argument(
          "The maximum number of iterations cannot be 0.");
